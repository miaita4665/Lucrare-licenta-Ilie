  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const uniqueHotels = [...new Map(hotelItems.map(i => [i.hotel.location, i.hotel])).values()]

        const hotelPins = await Promise.all(
          uniqueHotels.map(async (hotel) => {
            try {
              const r = await fetch(`/cities/match?name=${encodeURIComponent(hotel.location)}`)
              if (!r.ok) return null
              const city = await r.json()
              return { type: "hotel", hotel, lat: parseFloat(city.lat), lon: parseFloat(city.lon) }
            } catch { return null }
          })
        )
        ...
        const flightItems = bookings.flatMap(b =>
          b.items.filter(i => i.item_type === "Flight" && i.flight?.segments?.length > 0)
        )
          ...
          flightItems.map(async (item) => {
            ...
            try {
              const [fromRes, toRes] = await Promise.all([
                fetch(`/airports/search?q=${encodeURIComponent(seg.origin_code)}`).then(r => r.json()),
                fetch(`/airports/search?q=${encodeURIComponent(seg.destination_code)}`).then(r => r.json()),
              ])
              ...
              return {
                ...
                from: { code: seg.origin_code, lat: parseFloat(from.lat), lon: parseFloat(from.lon) },
                to: { code: seg.destination_code, lat: parseFloat(to.lat), lon: parseFloat(to.lon) },
              }
            } catch { return null }
          })
        )
        ...
        const firstPoint = validHotels[0] ?? (validFlights[0] ? { lat: validFlights[0].from.lat, lon: validFlights[0].from.lon } : null)
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="flex items-center gap-4 mb-6">
        <span className="flex items-center gap-1 text-slate-400 text-sm">
          <span className="w-3 h-3 rounded-full bg-blue-500 inline-block"/> Hotels
        </span>
        <span className="flex items-center gap-1 text-slate-400 text-sm">
          <span className="w-3 h-3 rounded-full bg-yellow-500 inline-block"/> Flight origin
        </span>
        <span className="flex items-center gap-1 text-slate-400 text-sm">
          <span className="w-3 h-3 rounded-full bg-red-500 inline-block"/> Flight destination
        </span>
      </div>
