...
export default function Flights() {
  ...
  const [from, setFrom] = useState(
    state?.selectedFrom
      ? `${state.selectedFrom.code} — ${state.selectedFrom.city}`
      : ''
  );
  const [to, setTo] = useState(
    state?.selectedTo
      ? `${state.selectedTo.code} — ${state.selectedTo.city}`
      : ''
  );
  ...
  const fetchPreferences = async () => {
    try {
      ...
      bookings.forEach((booking) => {
        booking.items
          .filter((i) => i.item_type === 'Flight' && i.flight)
          .forEach((i) => {
            const seg = i.flight.segments?.[0];
            if (seg?.destination_code)
              destinations[seg.destination_code] =
                (destinations[seg.destination_code] ?? 0) + 1;
            if (i.flight.airline_code)
              airlines[i.flight.airline_code] =
                (airlines[i.flight.airline_code] ?? 0) + 1;
          });
      });
  ...
  const runSearch = async (from, to, date) => {
    const [flightsRes, prefs] = await Promise.all([
      fetch(
        `/flights/search?from=${from.code}&to=${to.code}&date=${date}&fromCity=${encodeURIComponent(from.city)}&toCity=${encodeURIComponent(to.city)}`
      ).then((r) => r.json()),
      fetchPreferences(),
    ]);
    ...
    const scored = flightsRes.map((f) => ({
      ...f,
      _score:
        (prefs.destinations[f.to] ?? 0) + (prefs.airlines[f.airline] ?? 0),
    }));
    scored.sort((a, b) => b._score - a._score || a.price - b.price);
    setResults(scored);
  };

  // Auto-search if coming from landing page
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedFrom && selectedTo && date)
      runSearch(selectedFrom, selectedTo, date);
  };

  const hasPrefs =
    Object.keys(preferences.airlines).length > 0 ||
    Object.keys(preferences.destinations).length > 0;
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      {hasPrefs && (
        <div className="mt-4 flex flex-wrap gap-2 items-center">
          ...
          {Object.entries(preferences.airlines)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map(([a]) => (
              <span
          ...
          {Object.entries(preferences.destinations)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map(([d]) => (
              <span
                key={d}
                className="bg-slate-600 text-white text-xs px-2 py-1 rounded-full"
              >
                → {d}
              </span>
            ))}
      ...
      <div className="mt-8 grid grid-cols-1 gap-4">
        {results.map((flight) => (
          <div
            key={flight.id}
            className="bg-slate-800 p-4 rounded-lg text-white flex justify-between items-center"
          >
            <div>
