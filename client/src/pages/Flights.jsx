      ...
      {suggestions.length > 0 && (
        <ul className="absolute z-10 top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-600 rounded-lg overflow-hidden shadow-lg">
          {suggestions.map(airport => (
            ...
            >
              <div><span className="font-bold text-blue-400 mr-2">{airport.code}</span>{airport.city}</div>
...
export default function Flights() {
  ...
  const fetchPreferences = async () => {
    ...
    } catch { return { destinations: {}, airlines: {} } }
  }
        ...
        {results.map(flight => (
          <div key={flight.id} className="bg-slate-800 p-4 rounded-lg text-white flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <button onClick={() => navigate('/booking', { state: { flight } })} className="mt-2 bg-blue-600 px-4 py-2 rounded">Book</button>
            </div>
          </div>
        ))}
      </div>
